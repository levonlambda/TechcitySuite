...
class TransactionDetailsActivity : AppCompatActivity() {
    ...
    private fun showConfirmationDialog(
        ...
        val marginInDp = when (transactionType) {
            "Cash In" -> 135      // Keep current (works well)
            "Cash Out" -> 130     // Keep current (works well)
            "Misc Payment" -> 110  // Increased for better centering
            "Skyro Payment" -> 90  // Keep current (works well)
            "Mobile Loading Service" -> 60// Reduced to prevent text wrapping
            "Home Credit Payment" -> 60    // Keep current (works well)
            else -> 95
        }
        ...
        // Set fee option (without parentheses)
        dialogFeeOption.text = feeOption

        // Set customer receives message with colored parts
